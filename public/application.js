...
$(document).on('show.bs.modal', function (e) {
  var blogId = e.relatedTarget.dataset.id;
  console.log(blogId);
...
var config = {
  apiOrigins: {
    production: 'https://asiannerdist.herokuapp.com/'
  }
};
...
var onSubmitBlog = function onSubmitBlog(event) {
  event.preventDefault();
  var title = $('#new-blog-title').val();
  var content = $('#new-blog-content').val();
  api.adminBlog(title, content) // update api
  .then(ui.createSuccess) // update ui with
  .catch(ui.loopBlogsFailure);
};

var onBlogHistory = function onBlogHistory(event) {
  event.preventDefault();
  api.blogHistory().then(ui.loopBlogsSuccess).catch(ui.loopBlogsFail);
};

var onBlogEdit = function onBlogEdit(event) {
  console.log(event.target.dataset.id);
  event.preventDefault();
  var eventNum = event.target.dataset.id;
  var blogId = parseInt(eventNum);
  var title = $('#blog-title').val();
  var content = $('#content-text').val();
  api.updateBlog(blogId, title, content).then(ui.editBlogSuccess).catch(ui.editBlogFail);
};

var onBlogDelete = function onBlogDelete(event) {
  console.log(event);
  event.preventDefault();
  api.deleteBlog(event.target.dataset.id).then(ui.deleteBlogSuccess).catch(ui.deleteBlogFail);
};
...
var adminBlog = function adminBlog(title, content) {
  return $.ajax({
    url: app.host + 'blogs/',
    headers: {
      Authorization: 'Token token=' + app.user.token
...
var blogHistory = function blogHistory() {
  return $.ajax({
    url: app.host + 'blogs/',
    method: 'GET',
    headers: {
...
var updateBlog = function updateBlog(id, title, content) {
  return $.ajax({
...
var loopBlogsFail = function loopBlogsFail(error) {
  console.error(error);
};

var loopBlogsSuccess = function loopBlogsSuccess(data) {
  console.log(data);
  var blogs = data.blogs;
  $('#show-blogs').show();
  $('#show-blogs').empty();
  blogs.forEach(function (blog) {
    $('#show-blogs').append('<div style="width: 80%; background-color: white; color: black; padding: 0 0 23px 0; margin: 10px;">' + '<div style="background-color: #bdc3c7; color: black;">' + ('<h1> ' + blog.title + ' </h1>') + '</div>' + '<div style= padding 10px;>' + ('<p> ' + blog.content + ' </p>') + ('<input type="submit" class="btnDeleteBlog" name="delete" data-id="' + blog.id + '" value="delete blog">') + (
    // outside edit blog button
    '<button type="button" class="btnEditBlog" id="editBtn' + blog.id + '" data-toggle="modal" data-target="#exampleModal" data-id="' + blog.id + '">edit blog</button>') + '</div>' + '</div>');
  });
};

var editBlogSuccess = function editBlogSuccess(data) {
  console.log(data);
};

var editBlogFail = function editBlogFail(error) {
  console.error(error);
};

var deleteBlogSuccess = function deleteBlogSuccess(data) {
  console.log(data);
};

var deleteBlogFail = function deleteBlogFail(error) {
  console.error(error);
};
...
module.exports = {
  ...
  loopBlogsSuccess: loopBlogsSuccess,
  loopBlogsFail: loopBlogsFail,
  editBlogSuccess: editBlogSuccess,
  editBlogFail: editBlogFail,
  deleteBlogSuccess: deleteBlogSuccess,
  deleteBlogFail: deleteBlogFail
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
